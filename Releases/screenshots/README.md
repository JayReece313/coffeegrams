...
1.2 iPad pass. **`01-home.png` and `04-paywall.png` were recaptured and
visually verified for 1.2** (2026-09-01, alongside `02-calculator.png` and
`03-guided-timer.png` — see `submission_1.2.md`'s screenshot audit); all four
came back showing the same content as before, since neither iPad support nor
the rating prompt changes iPhone's visible UI. Only `05-brew-log.png` is a
real, already-known behavior change if rerun on iPhone: the new
